...
const symbol = urlParams.get('symbol')?.toLowerCase();
const number = urlParams.get('number');

// Find element based on query parameter
let result = null;
if (name) {
    result = elements.find(element => element.name.toLowerCase() === name);
} else if (symbol) {
    result = elements.find(element => element.symbol.toLowerCase() === symbol);
} else if (number) {
    result = elements.find(element => element.atomicNumber === parseInt(number));
}

// Return JSON response
document.write(JSON.stringify(result || { error: "Element not found" }, null, 2));
